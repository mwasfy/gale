...
csr_array *parseMM(char* tmpchar, int *p_num_nodes, int *p_num_edges, bool directed, bool weight_flag)
{
	...
	//printf("MODIFIED MM GRAPH PARSING\n");
    int cnt = 0;
    unsigned int lineno = 0;
    char line[128];
    ...
    while (fgets(line, 100, fptr)) {
        ...
        if (lineno > 0) {
			...
			//printf("VALUES READ HEAD: %d   TAIL: %d\n",head,tail);
	...
	//printf("TUPPLE ARRAY count %d\n",cnt);
    std::stable_sort(tuple_array, tuple_array + num_edges, doCompare);

#ifdef VERBOSE
    for (int i = 0 ; i < num_edges; i++) {
        printf("TUPPLE ARRAY[%d]: %d, %d, %d\n", i, tuple_array[i].row, tuple_array[i].col, col_cnt[tuple_array[i].row]);
    }
#endif
	int test_sum=0;
    for (int i = 0 ; i <= num_nodes; i++) {
...
/* 	printf("PRINTING CSR ARRAY\n");
	for (int i = 0 ; i < num_nodes; i++) {
        printf("row[%d] = %d --> column count = %d\n",i, row_array[i],col_cnt[i]);
    } */
...
csr_array *parseMM_transpose(char* tmpchar, int *p_num_nodes, int *p_num_edges, bool directed, bool weight_flag)
{
	...
	printf("TRANSPOSED MM GRAPH PARSING\n");
    int cnt = 0;
    unsigned int lineno = 0;
    char line[128];
    int num_nodes = 0, num_edges = 0, num_nodes2 = 0;
