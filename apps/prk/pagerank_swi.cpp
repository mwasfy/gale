//This is the snglw work item implementation of the pagerank algorithm (SPMV-based)
...
void create_kernels(){
	char * kernelprk1  = "spmv_csr_scalar_kernel";
	char * kernelprk2  = "pagerank2";
...
void parse_mem_alloc(){
	...
	data = (float *)malloc(num_edges * sizeof(float));
	if (!data) fprintf(stderr, "malloc failed data\n");
	
	for(int i=0; i < num_nodes; i++){
        pagerank_array[i] = 1 / (float)num_nodes;
        pagerank_array2[i] = 0.0f;
    }
    
    NORM_FACTOR = 0.15f/(float)num_nodes;
}
...
void exe_fpga(){
	...
	{
		int start = csr->row_array[id];
        int end;
        if (id + 1 < num_nodes)
           end = csr->row_array[id + 1] ;
        else
           end = num_edges;
	    ...
	    for(int edge = start; edge < end; edge++){
            nid = csr->col_array[edge];
            //Each neighbor will get equal amount of pagerank
	        data[edge] = 1.0/(float)csr->col_cnt[nid];
	...
	//kernel 3
	clSetKernelArg(kernel1, 0, sizeof(cl_int), (void*) &num_nodes);
	clSetKernelArg(kernel1, 1, sizeof(void *), (void*) &row_d);
	clSetKernelArg(kernel1, 2, sizeof(void *), (void*) &col_d);
	clSetKernelArg(kernel1, 3, sizeof(void *), (void*) &data_d);
	clSetKernelArg(kernel1, 4, sizeof(void *), (void*) &pagerank_d1);
	clSetKernelArg(kernel1, 5, sizeof(void *), (void*) &pagerank_d2);
	...
	for(int i = 0; i < ITER ; i++){
		...
		if(err != CL_SUCCESS) { fprintf(stderr, "ERROR: kernel2  clEnqueueNDRangeKernel()=>%d failed\n", err); }
	}
...
void print_time(){
	...
	printf("Kernel1: spmv_csr_scalar_kernel = %lf ms\n",timer_k1*1000);
	printf("Kernel2: pagerank2 = %lf ms\n",timer_k2*1000);
	printf("D2H: Data read = %lf ms\n",timer_d2h*1000);
	printf("Overhead: workload dist. overhead = %lf ms\n",timer_overhead*1000);

	double timer_total = 0;
	double timer_total_overhead = 0;
	timer_total = timer_h2d + timer_k1 + timer_k2 + timer_d2h;
